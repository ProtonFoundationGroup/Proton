...
func chiefAddressList() (list ChiefInfoList) {
	...
	if IsTestnet() {
		list = ChiefInfoList{
			...
			newChiefInfo(TestnetChainConfig.Chief001Block, "0.0.1", TestnetChainConfig.Chief001Address, TribeChief_0_0_1ABI),
		}
	} else {
		list = ChiefInfoList{
			// at same account and block number to deploy this contract can be get the same address
			newChiefInfo(MainnetChainConfig.Chief001Block, "0.0.1", MainnetChainConfig.Chief001Address, TribeChief_0_0_1ABI),
		}
	}
	chiefInfoList = list
	return
