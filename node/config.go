...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of geth is "proton". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Proton", keep that.
	if name == "proton" || name == "proton-testnet" {
		name = "Proton"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "proton" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	if c.name() == "proton" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "proton" {
...
func (c *Config) loadECDSAWithPwd() (*ecdsa.PrivateKey, error) {
	...
	overFunc := func(){
		fmt.Println("=================================================")
		fmt.Println(`> Must be first execute "proton security --unlock" `)
		fmt.Println("=================================================")
		os.Exit(-1)
	}
